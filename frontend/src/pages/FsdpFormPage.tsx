...
const newGrant = (): ParticipationInput => ({ program_name: "", delivering_hei: "", start_date: null, end_date: null, grant_type: null, grant_other: "", status: "On Going", extension: "", remarks: "" });
...
export default function FsdpFormPage() {
  ...
  useEffect(() => {
    if (!id) return;
    getFsdpRecord(Number(id)).then((scholar) => setForm({ name: scholar.name, age: scholar.age, previous_degree: scholar.previous_degree || "", missing_requirements: scholar.missing_requirements, department: scholar.department || "", rank: scholar.rank || "", tenure: scholar.tenure || "", participations: scholar.participations.map(({ name, delivering_hei, start_date, end_date, grant_type, grant_other, status, extension, remarks }) => ({ program_name: name, delivering_hei: delivering_hei || "", start_date, end_date, grant_type, grant_other: grant_other || "", status, extension: extension || "", remarks: remarks || "" })) })).catch((caughtError) => setError(caughtError.message));
  }, [id]);
